...
def fitness(genome: Genome, things: [Thing], weight_limit: int) -> int:#to evaluate solution
    if len(genome) != len(things):
        raise ValueError("genome and things must be of same length")

    weight = 0
    value = 0
    for i, thing in enumerate(things):#checking that either the genome is 1 or 0
        if genome[i] == 1:
            weight += thing.weight
            value += thing.value
